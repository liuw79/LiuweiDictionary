...
-(id) initAddressBookFromFile
{
    ...
    if (self)
    {
        ...
        _dictAddressBook = [NSKeyedUnarchiver unarchiveObjectWithData:addressData];
    }
...
-(void) listByGroup:(enum enumGroup) group
{
    @autoreleasepool
    {
        ...
        //NSArray *namesArray = [namesSet sortedArrayUsingDescriptors:[NSSortDescriptor sortDescriptorWithKey:@"name" ascending:YES]];
        //NSArray *namesArray = [[NSArray arrayWithObjects:
                            //    [namesSet allObjects], nil] sortedArrayUsingSelector:@selector(caseInsensitiveCompare:)];

        for (NSString *name in namesSet)
        {
            NSLog(@"%@", name);
            NSLog(@"Group %d list is : %@", group, _dictAddressBook[name]);
        }
    }
}
